...
func (d *AnswerItemModel) BeforeCreate(tx *gorm.DB) (err error) {
	if d.ID == uuid.Nil {
		d.ID = uuid.New()
	}
...
func (d *AnswerItemModel) BeforeUpdate(tx *gorm.DB) (err error) {
	d.UpdatedAt = time.Now()
	// set data from jwt token
...
func (d *AnswerItemModel) BeforeDelete(tx *gorm.DB) (err error) {
